...
class WayfSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($fields_info as $field_name => $field_info) {
      if ($field_info->getType() == 'text') {
        $form['fields']['mapping'][$field_name] = array(
          ...
          '#title' => Html::escape($field_info->getLabel()),
    ...
    $active_list = json_decode($config->get('sp_organizations_active'), TRUE);
    ...
    $form['settings']['development']['development_debug'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Debug mode'),
      '#default_value' => $config->get('development_debug'),
      '#description' => $this->t('By-pass WAYF services (mock) and return valid user object.'),
    );
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->config('wayf_dk_login.settings')
      ...
      ->set('development_debug', $form_state->getValue('development_debug', FALSE))
      ->save();
