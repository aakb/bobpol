    ...
    try {
      ...
      if ($sporto->isLoggedIn()) {
        ...
        if (!empty($destination)) {
          ...
          drupal_set_message(t("NemID failed. Please contact the site administrator or try again."), 'error');
          \Drupal::logger('wayf_dk_login')->error(t("NemID failed. The session did not contain any redirect information."));
        }
      }
    }
    catch (SPortoException $exception) {
